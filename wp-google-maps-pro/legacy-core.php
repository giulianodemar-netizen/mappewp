...
function wpgmza_show_ugm_incompatible_notice()
{
	...
	<div class="notice notice-error">
		<p>
			<?php
			_e('<strong>WP Go Maps Pro:</strong> User Generated Markers add-on 3.01 and below is not compatible with OpenLayers. Please either switch engine to Google under Maps &rarr; Settings, or update User Generated Markers to 3.02 or above', 'wp-google-maps');
			?>
		</p>
	</div>
	<?php
...
function wpgmza_show_gold_incompatible_notice()
{
	...
	<div class="notice notice-error">
		<p>
			<?php
			_e('<strong>WP Go Maps Pro:</strong> Gold Add-on versions 4.11 and below are not compatible with OpenLayers. Please update to Gold 4.11 or above to use Gold features with the OpenLayers engine.', 'wp-google-maps');
			?>
		</p>
	</div>
	<?php
...
wpgmza_require_once($plugin_dir_path . 'includes/3rd-party-integration/class.woocommerce-products.php');
...
function wpgmza_update_basic_v6_notice() {
	...
	<div class="notice notice-warning is-dismissible">
		<p>
			<?php
			_e('<strong>WP Go Maps Pro:</strong> Experiencing issues? We strongly recommend that you update WP Go Maps (Basic) to Version 9.0.0 in the plugins menu', 'wp-google-maps');
			?>
		</p>
	</div>
	<?php
...
function wpgmza_register_pro_version() {
	...
	if(version_compare((string) $wpgmza_version, '7.0.0', '<'))
...
function wpgmaps_tag_pro( $atts ) {
	...
	/*
	 * Legacy loads this on non admin pages, but it shouldn't 
	 * 
	 * This is being changed, but will be reversable, in case it is required
	*/
	...
	if(preg_match('/data-settings=".+"/', $html, $wpgmzaM) || preg_match('/data-settings=\'.+\'/', $html, $wpgmzaM))
	{
		$map_attributes = $wpgmzaM[0];
	}
	else
	{
	...
	if(preg_match('/data-shortcode-attributes=".+"/', $html, $wpgmzaM) || preg_match('/data-shortcode-attributes=\'.+\'/', $html, $wpgmzaM))
	{
		$map_attributes .= ' ' . $wpgmzaM[0];
	}
	else
	...
	if(isset($map_other_settings['list_markers_by']) && $map_other_settings['list_markers_by'] == '6') {
		...
		wp_enqueue_script('wpgmza_dummy', plugin_dir_url(WPGMZA_PRO_FILE) . 'dummy.js', false, false, $scriptArgs);
		
		wp_localize_script('wpgmza_dummy', 'wpgmza_modern_marker_listing_marker_order_by_id_for_map_' . (int)$atts['id'], $marker_id_order);
		
	    /* Developer Hook (Action) - Legacy hook which shared the marker order as part of the pro shortcode output */     
...
function wpgmaps_user_javascript_pro($atts = false) {
	...
	if (empty($wpgmza_settings['wpgmza_do_not_enqueue_datatables'])) {
		wp_register_script('wpgmaps_datatables', plugins_url(plugin_basename(dirname(__FILE__)))."/js/jquery.dataTables.min.js", true, false, $scriptArgs);
		wp_enqueue_script( 'wpgmaps_datatables' );
		
		wp_register_script('wpgmaps_datatables-responsive', plugins_url(plugin_basename(dirname(__FILE__)))."/js/dataTables.responsive.js", true, false, $scriptArgs);
	...
	/* Developer Hook (Action) - Enqueue additional scripts in Pro */     
	...
	if ( function_exists( "wpgmaps_ugm_activate" ) ) {
		...
		if ($wpgmza_vgmc < 300) {
			...
			wp_enqueue_script('wpgmaps_ugm_core', plugins_url('wp-google-maps-ugm') .'/js/ugm-core.js', array('wpgmaps_core'), $wpgmza_ugm_version.'vgm' , $scriptArgs);
...
function wpgmaps_upload_csv() {
    if (!function_exists("wpgmaps_activate")) {
        //echo "<div id='message' class='updated' style='padding:10px; '><span style='font-weight:bold; color:red;'>".__("WP Go Maps","wp-google-maps").":</span> ".__("Please ensure you have <strong>both</strong> the <strong>Basic</strong> and <strong>Pro</strong> versions of WP Go Maps installed and activated at the same time in order for the plugin to function correctly.","wp-google-maps")."<br /></div>";
    }
...
function wpgmza_cURL_response_pro($action) {
    if (function_exists('curl_version')) {
        global $wpgmza_pro_version;
        global $wpgmza_pro_string;
        $request_url = "https://www.wpgmaps.com/api/rec.php?action=$action&dom=".$_SERVER['HTTP_HOST']."&ver=".$wpgmza_pro_version.$wpgmza_pro_string;
        $ch = curl_init();
...
function wpgmza_pro_advanced_menu() {
    ...
    if(!$wpgmza->internalEngine->isLegacy()){
    	/* 
    	 * We will bail early here
    	 *
    	 * We will allow the new AdvancedPage class to manage things for us
    	 *
    	 * We need to make sure things here work normally for legacy users, but realistically adding this scalability now is a good things for us in the long run
    	 *
    	 * Done via an action, so we don't need to concern ourselves with scope too much
    	*/

	    /* Developer Hook (Action) - Tools page creation hook, for atlas novus only */     
    	do_action("wpgmza_advanced_page_create_instance");
		...
		<div id="wpgmaps_tabs">
			<ul>
				<?php
					 ...
					 * @since 7.0.0
					 */
	    			/* Developer Hook (Action) - Add additional tabs to the tools page */     
			...
			<?php
				 ...
				 * @since 7.0.0
				 */
			    /* Developer Hook (Action) - Add additional tab content to the tools page */     
...
$wpgmaps_api_url = 'https://ccplugins.co/api-wpgmza-v9/';
...
function wpgmaps_check_for_plugin_update($checked_data) {
	...
	$request_string = apply_filters('wpgmza_plugin_api_packet', $request_string, 'pro');
...
function wpgmaps_plugin_api_call($def, $action, $args) {
	...
	if(empty($plugin_info) || empty($plugin_info->checked) || empty($plugin_info->checked[$wpgmaps_plugin_slug .'/'. $wpgmaps_plugin_slug .'.php'])){
		/* Something is wrong with the transient data */
		return false;
	}
	...
	if (is_wp_error($request)) {
		...
		$res->name = 'WP Go Maps - Pro add-on';
...
function wpgmza_content_filter($content) {
    ...
    if(!empty($lat) && !empty($lng)){
       ...
       $map_data = do_shortcode("[wpgmza id='{$parent_id}' lat='{$lat}' lng='{$lng}' parent_id='{$parent_id}' mark_center='true']");
    }   
...
function wpgmza_upload_base64_image()
{
	...
	/*
	 * Newly added in V9.0.0 
	 * Date: 2022-05-06
	 * 
	 * Note: This allows the new marker editor to store it's content to a custom directory, without needing a whole new uploader 
	 */
	$usingCustomFolder = false;
	if(!empty($_POST['folder'])){
		$folder = rtrim($_POST['folder'], "/") . "/";
		$folderPath = rtrim($upload_dir['basedir'], "/") . "/" . $folder;
		$folderUrl = rtrim($upload_dir['baseurl'], "/") . "/" . $folder;
		
		if(file_exists($folderPath)){
			$upload_path = $folderPath;
			$upload_path = str_replace('/', DIRECTORY_SEPARATOR, $upload_path) . DIRECTORY_SEPARATOR;
	...
	if(!empty($_POST['template']) && !empty($folder) && strpos($folder, 'icons') !== FALSE){
		/* 
		 * Template data for an icon creation upload 
		 * Honestly, this whole thing should be rebuild to a REST endpoint. (Noted: 2024-04-17)
		*/
		$templatePath = rtrim($upload_dir['basedir'], "/") . "/" . $folder . "templates";
		if(file_exists($templatePath)){
			$templatePath = str_replace('/', DIRECTORY_SEPARATOR, $templatePath) . DIRECTORY_SEPARATOR;
			$templateName = explode('.', $filename);
			if(!empty($templateName)){
				$templateName = array_shift($templateName);
				$templateName = "{$templateName}.json";

				try{
					$templateJson = stripslashes(trim($_POST['template']));
					$templateJson = json_decode($templateJson);

					if(!empty($templateJson)){
						$templateJson = json_encode($templateJson);
						file_put_contents($templatePath . $templateName, $templateJson);
					}
				} catch (Exception $ex){
					/* Ignore */
				} catch (Error $err){
	...
	if($usingCustomFolder && !empty($folderUrl)){
		/* Simulates side load, but with support for the custom folder */
		$proxyResults = array(
			'file' => $tmp_name,
			'type' => "image/png",
		...
		$attachment	= array(
			'post_title' 		=> basename($proxyResults['url']),
			'post_content'		=> '',
			'post_status'		=> 'inherit',
			'post_mime_type'	=> $proxyResults['type']
		);
		...
		$meta_data = wp_generate_attachment_metadata($attachment_id, $proxyResults['file']);
		
		wp_update_attachment_metadata($attachment_id, $meta_data);
		wp_send_json($proxyResults);
	} else {
		$file = array(
			'error'		=> 0,
			'tmp_name'	=> $tmp_name,
...
function wpgmaps_get_import_logs(){
	...
	if(file_exists($importLogPath)){
		try{
			...
			if(filesize($importLogPath) > $fileSizeLimit || !empty($_GET['clearlogs'])){
				// File has gotten excessively large, destory the file now
				unlink($importLogPath);
				return "No logs found...";
			} else {
				return htmlentities(file_get_contents($importLogPath));
			}
		} catch (Exception $ex){
