...
class ProMap extends Map
{
	...
	public function __construct($id_or_fields=-1, $overrides=null)
	{
		...
		if(empty($wpgmza->settings['wpgmza_do_not_enqueue_owl_carousel'])){
			...
			wp_enqueue_script('owl-carousel', 						$base . 'lib/owl.carousel.js', array('jquery'), $wpgmza->getProVersion(), $scriptArgs);
		...
		wp_enqueue_script('featherlight',				$base . 'lib/featherlight.min.js', array('jquery'), $wpgmza->getProVersion(), $scriptArgs);
		...
		wp_enqueue_script('polyline',					$base . 'lib/polyline.js', array(), $wpgmza->getProVersion(), $scriptArgs);
		...
		if(is_admin() && !empty($this->fusion))
		{
			add_action('admin_notices', function() {
				...
				<div class="notice notice-error is-dismissible">
					<p>
						<?php
						_e('<strong>WP Go Maps:</strong> Fusion Tables are deprecated and will be turned off as of December the 3rd, 2019. Google Maps will no longer support Fusion Tables from this date forward.', 'wp-google-maps');
						?>
					</p>
				</div>
		...
		$this->_categoryLegends = CategoryLegends::createInstance($this);
	...
	public function duplicate()
	{
		...
		$types = array(
			...
			$WPGMZA_TABLE_NAME_MAPS_HAS_CUSTOM_FIELDS_FILTERS	=> null,	
			$WPGMZA_TABLE_NAME_CATEGORY_MAPS					=> null,
			$WPGMZA_TABLE_NAME_POINT_LABELS 					=> null,
			$WPGMZA_TABLE_NAME_IMAGE_OVERLAYS 					=> null,
