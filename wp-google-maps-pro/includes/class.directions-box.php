...
class DirectionsBox extends DOMDocument
{
	public function __construct($map, $shortcodeAttributes=null)
	{
		...
		$this->loadPHPFile($wpgmza->internalEngine->getTemplate('directions-box.html.php', WPGMZA_PRO_DIR_PATH));
		...
		if($outputPanel = $element->querySelector('.wpgmza-directions-output-panel')){
			$outputPanel->setAttribute('id', "directions_panel_{$map->id}");
		}

		if($notificationPanel = $element->querySelector('.wpgmza-directions-notifications')){
			$notificationPanel->setAttribute('id', "wpgmaps_directions_notification_{$map->id}");
		}
		...
		if($wpgmza->settings->useLegacyHTML){
			...
			if(!empty($map->directions_box_style))
			{
				$element->addClass('style-' . $map->directions_box_style);
			}
			...
			if(!empty($map->directions_box_style) && ($map->directions_box_style == "legacy" || $map->directions_box_style == "modern"))
			{
	    ...
	    /* Developer Hook (Action) - Alter output of the directions box, passes DOMElement for mutation */     
		do_action("wpgmza_directions_box_created", $element);
	}
}
