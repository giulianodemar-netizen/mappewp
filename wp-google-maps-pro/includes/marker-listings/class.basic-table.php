...
class BasicTable extends \WPGMZA\MarkerListing
{
	...
	public function getAjaxResponse($request)
	{
		...
		if(!empty($wpgmza->settings->markerlist_image_placeholder)){
			if($imagePlaceholder = $template->querySelector('.wpgmza_map_image')){
				$imagePlaceholder->setAttribute('src', $wpgmza->settings->markerlist_image_placeholder);
			}
		}
