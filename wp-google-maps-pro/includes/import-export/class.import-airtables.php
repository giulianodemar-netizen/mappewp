...
class ImportAIRTABLES extends Import
{
	 ...
	 * 
	 * Airtables only load 100 rows at a time, so we need this custom loader
	 */
	protected function load_file() {
		...
		if (!empty( $this->file_url ) ) {
			...
			do{
				...
				$batch = $this->fetchBatch($this->file_url, $offset);

				$hasNext = false;
				$offset = false;

				if(!empty($batch) && !empty($batch['records'])){
					$this->file_data[] = $batch;
					
					if(!empty($batch['offset'])){
						/* We found an offset */
						$hasNext = true;
						$offset = $batch['offset'];
					}
	...
	public function fetchBatch($url, $offset = false){
		if(!empty($url)){
			if(!empty($offset)){
				$url .= "&offset={$offset}";
			}

			$personalAccess = (object) array(
				'url' => false,
				'params' => false
			);
			...
			try{
				...
				if(!empty($query) && !empty($query['api_key'])){
					/* We have the api key in the params, let's prep for a personal access token request */
					$personalAccess->url = substr($url, 0, strpos($url, '?'));

					$personalAccess->params = array(
						'headers'     => array(
							'Authorization' => 'Bearer ' . $query['api_key']
						)
					);

					/* Now drop the API key and append any existing query params */
					unset($query['api_key']);
					if(!empty($query)){
						$query = http_build_query($query);
						$personalAccess->url .= "?{$query}";
					}
			...
			$forceLegacyRequest = false; 
			if(!empty($personalAccess->url)){
				/* We have a new personal access token style request */
				$this->log("Loading URL (+ Personal Access Airtable Token) {$personalAccess->url}");
				$content = wp_remote_get($personalAccess->url, $personalAccess->params);
				
				if(is_wp_error($content)){
					/* Older API key method, valid unit Jan 2024 */
					$forceLegacyRequest = true;
				} else {
					/* Now double check that the request didn't just come back with an auth error */
					$validityCheck = wp_remote_retrieve_body($content);
					if(!empty($validityCheck)){
						try{ 
							$validityCheck = json_decode( $validityCheck, true );
							if(!empty($validityCheck) && !empty($validityCheck['error'])){
								$forceLegacyRequest = true;
							}
						} catch(\Exception $ex){
							/* Exception, leave it */
			...
			if(!empty($forceLegacyRequest)){
				/* Older API key method, valid unit Jan 2024 */
				$this->log("Loading URL (Legacy Airtable API) {$url}");
			...
			if(!is_wp_error($content)){
				...
				$filePart = wp_remote_retrieve_body( $content );
				if(!empty($filePart)){
					try{ 
						$filePart = json_decode( $filePart, true );
						return $filePart;
					} catch(\Exception $ex){
						/* Exception, leave it */
					} catch(\Error $err){
				...
				$this->log("Failed to retrieve data from URL: " . $content->get_error_message());
			}
		}
		return false;
	}
	...
	protected function parse_file() {
		...
		if(!empty( $this->file_data)){
			...
			foreach($this->file_data as $data){
				...
				foreach ($data['records'] as $field_data) {
	...
	protected function import_markers() {
		...
		foreach($this->file_data['markers'] as $fields){
			...
			/* Custom field support (marker fields) */
			$customFields = array();
			foreach($fields as $fieldName => $fieldValue){
				$originalName = $fieldName;
				$fieldName = str_replace(array("custom field:", "marker field:"), "field:", strtolower($fieldName));
					...
					foreach($customFields as $fieldName => $fieldValue){
						if(!empty($fieldValue)){
							if(is_numeric($fieldName)){
								$fieldId = intval($fieldName);
								$instance->customFields->{$fieldId} = $fieldValue; 
							} else {
								$slug = str_replace(" ", "_", $fieldName);
								$instance->customFields->{$slug} = $fieldValue;
							}
						}
					}
				}
			...
			(function($) {
				...
				$('#import-csv').click(function(){
					...
					if($("#import_loader").hasClass('wpgmza-import-loader')){
						...
						/* Old approach */
						$('#import_loader_text').html('<br/><?php echo wp_slash( __( 'Importing, this may take a moment...', 'wp-google-maps' ) ); ?><br/><progress id="wpgmza-import-csv-progress"/>');
					}
