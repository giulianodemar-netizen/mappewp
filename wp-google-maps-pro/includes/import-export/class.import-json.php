<?php
/**
 * WP Go Maps Pro Import / Export API: ImportJSON class
 *
...
/**
 * JSON importer for WP Go Maps Pro
 *
...
class ImportJSON extends Import {
	...
	protected function check_options() {
		...
		$this->options['pointlabels']   = isset( $this->options['pointlabels'] ) ? true : false;
		$this->options['imageoverlays'] = isset( $this->options['imageoverlays'] ) ? true : false;
	...
	protected function parse_file() {
		...
		$this->log("Attempting to parse JSON");
		...
		if ( ! empty( $this->file_data ) ) {
			...
			if( $this->file_data === null )
			{
				...
				throw new \Exception( __('Error parsing JSON: ', 'wp-google-maps') . json_last_error_msg() );
			}

			if(!empty($this->file_data['type']) && $this->file_data['type'] === "FeatureCollection"){
				throw new \Exception( __('It looks like you may be trying to import a GeoJSON file, please change the file extension from .json to .geojson', 'wp-google-maps') );
			}

			if ( empty( $this->file_data['maps'] ) )
			{
				$this->log("No maps detected");
	...
	public function admin_options() {
		...
		<div class="switch"><input id="pointlabels_import" class="map_data_import cmn-toggle cmn-toggle-round-flat" type="checkbox" <?php echo empty( $this->file_data['pointlabels'] ) ? 'disabled' : ( ! $doing_edit || $this->options['pointlabels'] ? 'checked' : '' ); ?>><label for="pointlabels_import"></label></div><?php esc_html_e( 'Point Labels', 'wp-google-maps' ); ?><br>
		<div class="switch"><input id="imageoverlays_import" class="map_data_import cmn-toggle cmn-toggle-round-flat" type="checkbox" <?php echo empty( $this->file_data['imageoverlays'] ) ? 'disabled' : ( ! $doing_edit || $this->options['imageoverlays'] ? 'checked' : '' ); ?>><label for="imageoverlays_import"></label></div><?php esc_html_e( 'Image Overlays', 'wp-google-maps' ); ?><br>
		<br>
		
		<div class="switch">
			<input 
				id="geocode_import" 
				class="map_data_import cmn-toggle cmn-toggle-round-flat" type="checkbox"
				<?php
				echo ($doing_edit && isset($this->options['geocode']) ? 'checked' : '');
				?>
				/>
			<label for="geocode_import"></label>
		</div>
		<?php esc_html_e( 'Find Addresses or Latitude and Longitude when missing', 'wp-google-maps' ); ?>
		<br>
		...
		<div id="import-schedule-json-options" <?php if ( ! $doing_edit ) { ?>style="display:none;"<?php } ?>>
			...
			<select id="import-schedule-json-interval" class="import-schedule-json-options">
				...
				foreach($internal_intervals as $alias => $interval){
					?>
						<option value="<?php echo esc_attr( $alias ); ?>" <?php echo $doing_edit && $alias === $this->options['interval'] ? 'selected' : ''; ?>>
			...
			(function($){
				...
				$('#import-schedule-json').click(function(){
					if ($('#import-json').is(':visible')) {
						...
						if($("#import_loader").hasClass('wpgmza-import-loader')){
							/* New loader */
							$('.wpgmza-import-loader .wpgmza-loader-message').text('<?php echo wp_slash( __( 'Scheduling, this may take a moment...', 'wp-google-maps' )); ?>');
							$('.wpgmza-import-loader .wpgmza-progress-bar').addClass('wpgmza-hidden');
						} else {
							/* Old approach */
	...
	public function import() {
		...
		for ( $i = 0; $i < $iterations; $i++ ) {
			...
			$this->import_pointlabels();
			$this->import_imageoverlays();
	...
	protected function import_maps() {
		...
		foreach ( $this->file_data['maps'] as $map ) {
			...
			if(!empty($map->filter_custom_field_names))
			{
				// TODO: Link
			}
		...
		foreach ( $this->file_data['categories'] as $cat_key => $category ) {
			...
			if ( ! isset( $this->cat_id_map[ $cat_id ] ) ) {
				...
				if ( ! empty( $category['category_name'] ) ) {
					...
					$cat_search = $wpdb->get_var( $wpdb->prepare( "SELECT `id` FROM `$wpgmza_tblname_categories` WHERE `category_name`=%s AND active=0", $category['category_name'] ) );
				...
				if ( null === $cat_search ) {
					...
					$success = $wpdb->insert( $wpgmza_tblname_categories, $params, array(
						'%d',
						'%s',
						'%s',
						'%d',
	...
	protected function import_markers() {
		...
		foreach ( $this->file_data['markers'] as $fields ) {
			...
			{
				//$this->log("Ignoring marker, no map found");
				//continue;
			}
			...
			if(!empty($fields['ratings']))
			{
				$ratings_data = $fields['ratings'];
				unset($fields['ratings']);
			}
			...
			if(!empty($custom_fields_data))
			{
				...
				foreach($custom_fields_data as $key => $value)
				{
					if(isset($value['id']))
					{
						// NB: New format with ID as a field
						$custom_field_name = $value['name'];
						
						unset($value['id']);
						unset($value['name']);
						
						$custom_fields->{$custom_field_name} = $value['value'];
					}
					else
					{
						// NB: Old format with field names as keys
			...
			// Ratings
			if($ratings_data)
				foreach($ratings_data as $arr)
				{
					// Discard the rating ID
					unset($arr['id']);
					...
					$stmt = $wpdb->prepare("
						INSERT INTO $WPGMZA_TABLE_NAME_RATINGS
						(
							created,
							amount,
							ip_address,
					...
					// Now link the rating to the marker
					$params = array(
					...
					$stmt = $wpdb->prepare("
						INSERT INTO $WPGMZA_TABLE_NAME_MARKERS_HAS_RATINGS
						(rating_id, marker_id)
						VALUES
						(%d, %d)
					", $params);
	 ...
	 * Import pointlabels.
	 */
	protected function import_pointlabels() {

		if ( ! $this->options['pointlabels'] || empty( $this->file_data['pointlabels'] ) ) {
		...
		foreach ( $this->file_data['pointlabels'] as $pointlabel ) {
			if ( ! isset( $pointlabel['map_id'] ) || absint( $pointlabel['map_id'] ) < 1 || ! isset( $this->map_id_map[ absint( $pointlabel['map_id'] ) ] ) ) {
				continue;
			}

			$success = $wpdb->query( $wpdb->prepare( "INSERT INTO `$WPGMZA_TABLE_NAME_POINT_LABELS`
				(`map_id`,`name`,`center`,`fillColor`,`lineColor`,`opacity`, `fontSize`)
				VALUES (%d,%s,POINT(%f,%f),%f,%s,%f,%d)",
				$this->map_id_map[ absint( $pointlabel['map_id'] ) ],
				isset( $pointlabel['name'] ) ? $pointlabel['name'] : __( 'New Imported Label', 'wp-google-maps' ),
				isset( $pointlabel['centerX'] ) ? $pointlabel['centerX'] : 0,
				isset( $pointlabel['centerY'] ) ? $pointlabel['centerY'] : 0,
				isset( $pointlabel['fillColor'] ) ? $pointlabel['fillColor'] : '#000000',
				isset( $pointlabel['lineColor'] ) ? $pointlabel['lineColor'] : '#ffffff',
				isset( $pointlabel['opacity'] ) ? $pointlabel['opacity'] : 1,
				isset( $pointlabel['fontSize'] ) ? $pointlabel['fontSize'] : 11
			) );
	...
	protected function import_polygons() {
		...
		foreach ( $this->file_data['polygons'] as $polygon ) {
			...
			$success = $wpdb->insert( $wpgmza_tblname_poly, array(
				...
				'description'   => isset( $polygon['description'] ) ? $polygon['description'] : '',
	...
	/**
	 * Import imageoverlays.
	 */
	protected function import_imageoverlays() {
		if ( ! $this->options['imageoverlays'] || empty( $this->file_data['imageoverlays'] ) ) {
			return;
		}
		...
		foreach ( $this->file_data['imageoverlays'] as $imageoverlay ) {
			if ( ! isset( $imageoverlay['map_id'] ) || absint( $imageoverlay['map_id'] ) < 1 || ! isset( $this->map_id_map[ absint( $imageoverlay['map_id'] ) ] ) ) {
				continue;
			}

			$success = $wpdb->query( $wpdb->prepare( "INSERT INTO `$WPGMZA_TABLE_NAME_IMAGE_OVERLAYS`
				(`map_id`,`name`,`cornerA`,`cornerB`,`image`,`opacity`)
				VALUES (%d,%s,POINT(%f,%f),POINT(%f,%f),%s,%f)",
				$this->map_id_map[ absint( $imageoverlay['map_id'] ) ],
				isset( $imageoverlay['name'] ) ? $imageoverlay['name'] : __( 'New Imported Image', 'wp-google-maps' ),
				isset( $imageoverlay['cornerAX'] ) ? $imageoverlay['cornerAX'] : 0,
				isset( $imageoverlay['cornerAY'] ) ? $imageoverlay['cornerAY'] : 0,
				isset( $imageoverlay['cornerBX'] ) ? $imageoverlay['cornerBX'] : 0,
				isset( $imageoverlay['cornerBY'] ) ? $imageoverlay['cornerBY'] : 0,
				isset( $imageoverlay['image'] ) ? $imageoverlay['image'] : '',
				isset( $imageoverlay['opacity'] ) ? $imageoverlay['opacity'] : 1
			) );
