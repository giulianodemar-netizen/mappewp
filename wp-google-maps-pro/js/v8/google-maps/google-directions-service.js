...
jQuery(function($) {
	...
	WPGMZA.GoogleDirectionsService = function(map)
	{
		...
		if(!WPGMZA.CloudAPI.isBeingUsed){
			if(WPGMZA.settings.googleForceLegacyDirectionsEnabled){
				/* Using legacy, now deprecated direction services */
				this.googleDirectionsService = new google.maps.DirectionsService();
			} else {
				/* Using a route API proxy class designed to allow the system to more easily switch */
				this.googleDirectionsService = new WPGMZA.GoogleRouteAPI();
			}
		}else{
		...
		this.googleDirectionsService.route(request, function(response) {
			...
			switch(response.status)
			{
				...
				case google.maps.DirectionsStatus.REQUEST_DENIED:
					status = WPGMZA.DirectionsService.REQUEST_DENIED;

					/* Let's try to fallback to the legacy system */
					if(self.googleDirectionsService instanceof WPGMZA.GoogleRouteAPI){
						console.warn("Automatically switching to legacy directions. We recommend adding the Routes API to your Google Maps Project for the latest features");
						WPGMZA.settings.googleForceLegacyDirectionsEnabled = true; //Flag it in settings
						
						self._fallbackLegacyDirectionsAPI = true;
						self.googleDirectionsService = new google.maps.DirectionsService();
